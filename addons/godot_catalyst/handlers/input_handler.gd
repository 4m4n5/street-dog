...
##
## When the game is running, all simulate_* calls route through the debugger
## bridge so events reach the game process. When only the editor is running,
## they fall back to Input.parse_input_event in the editor process.
...
func simulate_key(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		var payload: Array = [int(keycode), pressed, echo, shift, ctrl, alt]
		var resp = await _plugin.debugger_plugin.request("simulate_key", payload)
		return _bridge_result(resp, {"action": "press_and_release" if pressed == null else ("press" if pressed else "release"), "key": keycode_str})
...
func simulate_mouse(params: Dictionary) -> Dictionary:
	...
	var drag_end := Vector2(drag_to.get("x", pos.x), drag_to.get("y", pos.y))

	if _use_bridge():
		var payload: Array = [action, pos.x, pos.y, int(btn), scroll_delta, drag_end.x, drag_end.y, double_click]
		var resp = await _plugin.debugger_plugin.request("simulate_mouse", payload)
		return _bridge_result(resp, {"action": action, "position": {"x": pos.x, "y": pos.y}})
	...
	match action:
		...
		"drag":
			...
			move.position = drag_end
			move.global_position = drag_end
			move.relative = drag_end - pos
			Input.parse_input_event(move)
			var release := InputEventMouseButton.new()
			release.position = drag_end
			release.global_position = drag_end
			release.button_index = btn
			release.pressed = false
			Input.parse_input_event(release)
			return {"success": true, "action": "drag", "from": {"x": pos.x, "y": pos.y}, "to": {"x": drag_end.x, "y": drag_end.y}}
...
func simulate_touch(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		var payload: Array = [action, index, pos.x, pos.y]
		var resp = await _plugin.debugger_plugin.request("simulate_touch", payload)
		return _bridge_result(resp, {"action": action, "index": index, "position": {"x": pos.x, "y": pos.y}})
...
func simulate_gamepad(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		...
		if params.has("button"):
			kind = "button"
			index = int(params["button"])
			value = 1.0 if params.get("pressed", true) else 0.0
		elif params.has("axis"):
			kind = "axis"
			index = int(params["axis"])
			value = float(params.get("axis_value", 0.0))
		else:
			return _error(-32600, "Must provide 'button' or 'axis' parameter")
		var resp = await _plugin.debugger_plugin.request("simulate_gamepad", [device, kind, index, value])
		return _bridge_result(resp, {"type": kind, "device": device, "index": index, "value": value})
...
func simulate_action(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		var resp = await _plugin.debugger_plugin.request("simulate_action", [action_name, pressed, strength])
		return _bridge_result(resp, {"action": action_name, "type": "press_and_release" if pressed == null else ("press" if pressed else "release")})

	# Editor-side fallback: only works for actions in editor's InputMap (mostly ui_*)
	if not InputMap.has_action(action_name):
		return _error(-32001, "Input action not found: '%s' (game not running; editor InputMap lacks project-defined actions)" % action_name)
...
func replay(params: Dictionary) -> Dictionary:
	...
	for event_data in recording:
		if event_data is Dictionary:
			...
			match type:
				"key":
					await simulate_key(data)
				"mouse":
					await simulate_mouse(data)
				"touch":
					await simulate_touch(data)
				"gamepad":
					await simulate_gamepad(data)
				"action":
					await simulate_action(data)
			replayed += 1
...
func _use_bridge() -> bool:
	return _plugin != null and _plugin.debugger_plugin != null and _plugin.debugger_plugin.has_active_session()


func _bridge_result(resp: Variant, ok_extra: Dictionary) -> Dictionary:
	if resp == null:
		return _error(-32005, "Timed out waiting for game response")
	var r: Dictionary = resp
	var msg: String = r.get("message", "")
	if msg == "input_error":
		var data_arr: Array = r.get("data", [])
		var err_msg: String = str(data_arr[1]) if data_arr.size() > 1 else "unknown"
		return _error(-32003, err_msg)
	var result := {"success": true, "via": "bridge"}
	result.merge(ok_extra, true)
	return result
...
func _string_to_keycode(s: String) -> Key:
	match s.to_upper():
		...
		"GREATER", ">": return KEY_GREATER
		"LESS", "<": return KEY_LESS
	return KEY_NONE
