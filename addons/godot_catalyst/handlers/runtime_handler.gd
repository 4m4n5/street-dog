...
# --- runtime.eval ---
# Routes to the running game via the debugger bridge so code executes in the
# game's scene tree context. Falls back to editor-process execution when no
# game is running.
func eval(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		var resp = await _plugin.debugger_plugin.request("eval", [code])
		if resp == null:
			return _error(-32005, "Timed out waiting for game eval response")
		var r: Dictionary = resp
		var d: Array = r.get("data", [])
		var ok: bool = bool(d[1]) if d.size() > 1 else false
		var payload = d[2] if d.size() > 2 else ""
		if not ok:
			return _error(-32003, str(payload))
		return {"success": true, "result": str(payload), "context": "game"}
	...
	if obj.has_method("_run"):
		var result = obj._run()
		return {"success": true, "result": str(result), "context": "editor"}

	return _error(-32003, "Failed to execute code")


# --- runtime.get_tree ---
# Returns the real runtime scene tree from the running game when available,
# else returns the edited scene root from the editor.
func get_tree(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		var resp = await _plugin.debugger_plugin.request("get_tree", [max_depth, include_properties])
		if resp == null:
			return _error(-32005, "Timed out waiting for game tree response")
		var r: Dictionary = resp
		var d: Array = r.get("data", [])
		var ok: bool = bool(d[1]) if d.size() > 1 else false
		if not ok:
			return _error(-32003, str(d[2]) if d.size() > 2 else "unknown error")
		return {"success": true, "tree": d[2], "context": "game"}

	if not EditorInterface.is_playing_scene():
		var root := EditorInterface.get_edited_scene_root()
		if root == null:
			return _error(-32002, "No scene open in editor")
		var tree_data := _build_tree(root, 0, max_depth, include_properties)
		return {"success": true, "tree": tree_data, "context": "editor"}

	return _error(-32005, "Game is running but debugger bridge has no active session")


# --- runtime.inspect_node ---
# Inspects the running game's node when available, else the editor's scene.
func inspect_node(params: Dictionary) -> Dictionary:
	...
	if _use_bridge():
		var resp = await _plugin.debugger_plugin.request("inspect_node", [node_path, include_methods, include_signals, property_filter])
		if resp == null:
			return _error(-32005, "Timed out waiting for game inspect response")
		var r: Dictionary = resp
		var d: Array = r.get("data", [])
		var ok: bool = bool(d[1]) if d.size() > 1 else false
		if not ok:
			return _error(-32001, str(d[2]) if d.size() > 2 else "unknown error")
		return {"success": true, "node": d[2], "context": "game"}
	...
	return {"success": true, "node": info, "context": "editor"}
